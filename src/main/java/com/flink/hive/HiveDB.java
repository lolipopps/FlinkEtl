...
public class HiveDB extends BaseDB implements Serializable {
    ...
    private HiveCatalog getCatalog() {
        if (catalog == null) {
...
//                } catch (IOException e) {
//                    log.info("拷贝文件失败: " + e.toString());
//                }
//                hiveConfDir = localConfDir.getAbsolutePath();
                catalog = new HiveCatalog(dbName, dbName, hiveConfDir, hiveVersion);
                catalog.open();
    ...
    @Override
    public List<String> listTableNames() throws DatabaseNotExistException {
...
//        }
    ...
    public Table getStreamTable(String tableName, Params parameter, Long sessionId) throws Exception {
        ...
        MLEnvironmentFactory.get(sessionId).getStreamExecutionEnvironment();
